...
const vismin_node = mysql.createPool({
  ...
  user: process.env.VISMIN_DB_USER,
...
async function storeQuery(dbPool, query, content) {
  ...
  if (dbPool == central_node) {
    t_dest = 1;
  } else if (dbPool == luzon_node) {
    t_dest = 2;
  } else if (dbPool == vismin_node) {
    t_dest = 3;
  } else {
    t_dest = -1;
  ...
  let localQuery =
    "INSERT INTO appt_logs (pxid,clinicid,doctorid,apptid,status,TimeQueued,QueueDate,StartTime,EndTime,type,Virtual,t_type,t_dest) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  let localContent = [
    content.pxid,
    content.clinicid,
    content.doctorid,
...
async function commitTransaction(log, currnode) {
  let query =
    "INSERT INTO appt_main (pxid,clinicid,doctorid,apptid,status,TimeQueued,QueueDate,StartTime,EndTime,type,Virtual) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  let content = [
    log.pxid,
    log.clinicid,
    log.apptid,
